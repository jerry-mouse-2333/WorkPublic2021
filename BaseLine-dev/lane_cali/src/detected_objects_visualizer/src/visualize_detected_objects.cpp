  ...
  for (auto const &object: in_objects.objects)
  {
    if (IsObjectValid(object))
    {
      ...
      if(!object.label.empty() && object.label != "unknown")
        cout<<"lane_id in vis: "<<unsigned(object.lane_id)<<endl;
        label_marker.text =  "lane_id: " +to_string(object.lane_id); //Object Class if available
      ...
      if (object.velocity_reliable)
      {
        ...
        // std::string text = "\n<" + std::to_string(object.id) + "> " + kmh_velocity_stream.str() + " km/h"+"\n<" + " size_x:"+std::to_string(object.dimensions.x)+" size_y:"+std::to_string(object.dimensions.y);
        std::string text = "\n pos_x:"+std::to_string(object.pose.position.x)+" pos_y:"+std::to_string(object.pose.position.y);
